...
Soils <- readOGR(dsn = "C:/Mythings/PhD/Data/Soils/", layer = "Unst_soils")
Soils = spTransform(Soils, CRS("EPSG:27700"))
Soils2 <- as(Soils, "SpatialPolygons")
soil_tiles <- Soils@polygons
...
for (tile_number in 1:length(indiv_tiles)) {
  ...
  for (soil_type_number in 1:length(soil_tiles)) {
    answer <- point.in.polygon(Broch[tile_number, 5], Broch[tile_number, 6], soil_tiles[[soil_type_number]]@Polygons[[1]]@coords[, 1], soil_tiles[[soil_type_number]]@Polygons[[1]]@coords[, 2])
    if (answer == 1) {broch_soil <- Soils@data$LANDFORM[[soil_type_number]]
    broch_df_list[[tile_number]] <- cbind(broch_df_list[[tile_number]], Broch_Landform = broch_soil)
...
for (number in 1:length(broch_df_list)) {
  soils_sites <- rbind.fill(soils_sites, broch_df_list[[number]])
}


#Soils_Brochs_plot <- ggplot(data = soils_sites, aes(y = Dist_from_Broch, x= SiteType, col = SiteType)) + 
#  geom_point() + 
#  geom_rect(data = soils_sites, aes(fill = Broch_Landform), xmin = -Inf, xmax = Inf, ymin = - Inf, ymax = Inf, alpha = 0.3) +
#  theme(strip.background = element_rect(fill = soils_sites$Broch_Landform)) +
#  facet_wrap(~areas_sites$Broch) + 
#  labs(y = "Distance from Broch (metres)", x = "", col = "Sites") + coord_flip()
#ggsave("Soils_Brochs_plot.png", plot = Soils_Brochs_plot, width = 11.69, height = 8.27, units = "in")
